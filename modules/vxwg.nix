...
in
{
  ...
  config = mkIf cfg.enable {
    networking.wireguard.interfaces.mesh = {
      ...
      postSetup =
        (forEach
          (catAttrs "id" (attrValues (filterAttrs (k: _v: k != config.networking.hostName) cfg.peers)))
          (
            x:
            "${pkgs.iproute2}/bin/bridge fdb append 00:00:00:00:00:00 dev vmesh dst 100.88.0.${toString x} via mesh"
          )
        )
      ...
      }) (attrValues (filterAttrs (k: _v: k != config.networking.hostName) cfg.peers));
    };
  };
}
